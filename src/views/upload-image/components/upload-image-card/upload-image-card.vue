  ...
  >
    ...
    <div class="before-upload-handle-container">
      ...
      >
        ...
        <!-- 随机字符 -->
        <div class="operate-item" v-if="!imgObj.filename.isRename">
          <el-checkbox
            :label="$t('upload_page.add_random')"
            v-model="fileNameOperateData.isAddRandom"
            @change="onRandomNaming($event)"
          ></el-checkbox>
        </div>
...
const onRandomNaming = (e: boolean) => {
  addRandomHandle(e, props.imgObj)
}
...
const initFilename = () => {
  ...
  if (props.imgObj!.uploadStatus.progress === 0) {
    ...
    // 随机字符处理
    addRandomHandle(imageName.enableRandom, props.imgObj)
    ...
    fileNameOperateData.isAddRandom = props.imgObj!.filename.isAddRandom
