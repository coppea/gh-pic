 ...
 *            ?? 随机字符（开启随机字符时）
 *            ?? 原始上传文件名（默认）
 *   若开启添加前缀且未开启重命名，则在基础名称前追加前缀
 *   若开启哈希化，则在扩展名前插入哈希值
 *
 * @param filename 图片对象的 filename 字段
 * @returns {string} 最终文件名
 */
export const updateFilename = (filename: UploadImageModel['filename']): string => {
  let baseName: string

  if (filename.isRename && filename.newName) {
    baseName = filename.newName
  } else if (filename.isAddRandom) {
    baseName = filename.randomStr
  } else {
    baseName = filename.initName
  }

  if (filename.isAddPrefix && filename.prefix && !filename.isRename) {
    baseName = `${filename.prefix}${baseName}`
  }

  const hashPart = filename.isAddHash && filename.hash ? `.${filename.hash}` : ''
  filename.name = baseName
  filename.final = `${baseName}${hashPart}.${filename.suffix}`
